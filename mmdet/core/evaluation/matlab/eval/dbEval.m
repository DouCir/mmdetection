...
algs = algs(end);           % select one or more algorithms for evaluation % Yuan 2019/01/25
...
for g=1:nGt %for each experiment
  for d=1:nDt %for each algorithm
...
for p=1:nPlots
  ...
  if( plotAlg )
    ...
    styles1=styles(ord); f=fopen([fName1 '.txt'],'a+');
  ...
  for s=1:length(setIds)
    for v=1:length(vidIds{s})
      ...
      for f=skip-1:skip:A.nFrame-1
        bb = vbb('frameAnn',A,f+1,lbls,filterGt); ids=bb(:,5)~=1; %����λ��ʾ�Ƿ���ԣ�1��ignore;  0 : not ignore
        bb(ids,:)=bbApply('resize',bb(ids,:),1,0,aspectRatio);  %ΪʲôҪ��GTbox�������ĵ�����
        k=k+1; gt{k}=bb;
      end
    end
  end
...
%         bb=bbApply('resize',bb,resize,0,aspectRatio); k=k+1; dt{k}=bb;  %�Ͷ�GTbox����һ����Ҳ��DTbox���˵���
%       end
      for f=frames, bb=bbs(bbs(:,1)==f+1,2:6);
         if ~isempty(bb)
             bb(:,3)=bb(:,3)-bb(:,1)+1;
