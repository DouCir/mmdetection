...
class TwoStageDetectorYuan(BaseDetector, RPNTestMixin, BBoxTestMixin,
                           MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(TwoStageDetectorYuan, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(TwoStageDetectorYuan, self).init_weights(pretrained)
    ...
    def extract_feat(self, img):
        x_temp = self.backbone(img)
        if self.with_neck:
            x_fpn = self.neck(x_temp)
        x = []
        x.append(x_temp[2])
        return tuple(x), x_fpn
    ...
    def forward_train(self,
                      ...
                      gt_masks=None,
                      proposals=None):
        x, x_fpn = self.extract_feat(img)

        losses = dict()

        # RPN forward and loss
        if self.with_rpn:
            rpn_outs = self.rpn_head(x_fpn)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        proposal_list = self.simple_test_rpn(
            x_fpn, img_meta, self.test_cfg.rpn) if proposals is None else proposals
